#!/usr/bin/env python3
"""Weekly Thai news -> explained Level-constrained learning content for Thai Vocabulary Trainer Ver6.0.2."""
...
HEADERS = {"User-Agent": "ThaiVocabularyTrainer/6.0.2 (+educational weekly updater)"}
...
def validate(data, allowed):
    ...
    for i,item in enumerate(shorts):
        ...
        if not item.get("explanation"): problems.append(f"short_news[{i}] needs explanation")
        if len(item.get("breakdown") or []) < 2: problems.append(f"short_news[{i}] needs word breakdown")
        if not item.get("grammar_note"): problems.append(f"short_news[{i}] needs grammar_note")
        if not item.get("reading_tip"): problems.append(f"short_news[{i}] needs reading_tip")
        hints=item.get("choice_explanations") or {}
        for choice in item.get("choices") or []:
            if choice not in hints: problems.append(f"short_news[{i}] missing choice explanation: {choice}")
...
- EVERY short_news item must teach after answering, not just grade. Add:
  1) explanation: concise Japanese explanation of why the sentence means the correct answer.
  2) breakdown: 2-8 Thai words/chunks, each with thai, Japanese katakana reading, and Japanese meaning.
  3) grammar_note: a short Japanese explanation of the useful sentence pattern.
  4) reading_tip: a practical Japanese tip showing where to split the Thai sentence when reading.
  5) choice_explanations: an object with ALL 4 Japanese choice strings as keys and a short Japanese reason why each is correct or wrong. Do not invent facts; explain from the Thai text only.
...
{{
  ...
  "short_news":[{{"id":"...","level":{TARGET_LEVEL},"title":"...","thai":"...","japanese":"...","reading":"...","choices":["...","...","...","..."],"explanation":"Japanese explanation","breakdown":[{{"thai":"...","reading":"...","japanese":"..."}}],"grammar_note":"...","reading_tip":"...","choice_explanations":{{"choice text":"reason"}},"source_type":"news","source_name":"Thai PBS","source_title":"...","source_url":"exact supplied URL","published_at":"YYYY-MM-DD","ai_simplified":true}}],
